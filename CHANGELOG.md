...
- **Two new commands turn the advisor panel into a recurring coaching ritual.** `/obsidian-advisor [advisor + topic]` holds a live 1:1 with one of the vault's `Advisors/`: the advisor builds a 3 to 5 point agenda from the Second Brain (anchor projects, open tasks, recent daily-note and `log.md` progress), opens by checking the prior session's commitments, coaches in character within the board's scope limits, then saves a `type: advisor-session` note to `Advisors/Sessions/`, mints a `type: task` per commitment, and backlinks the previous session so accountability compounds week over week. `/obsidian-advisory-review [week]` runs the weekend synthesis across the week's session notes: advances, blockers, where advisors contradict each other, commitments kept vs dropped, and the top 3 priorities for next week, saved as `type: advisory-review` in `Advisors/Reviews/`. New `type:` schemas added to `references/ai-first-rules.md`: `advisor-session`, `advisory-review`. Command count: 36 → 38.
- **Six new commands for recurring workflows, derived from real vault activity.** `/obsidian-proposal` drafts an 8-section B2B commercial proposal and keeps a `type: opportunity` deal tracker in sync. `/obsidian-panel` convenes a panel of the vault's `Advisors/` and records one verdict per advisor plus a synthesis. `/obsidian-1on1` captures a student 1:1 and mints `ALE-###` learnings into the learnings log. `/obsidian-event` creates an event ops note (pre / day-of / post). `/obsidian-recurring` tracks a recurring obligation with a cadence and computed `next-due`. `/obsidian-launch-block` creates a mother-task with an internal checklist (one node with `depends-on` in the Task Graph plugin). New `type:` schemas added to `references/ai-first-rules.md`: `proposal`, `opportunity`, `advisor-panel`, `student-meeting`, `event`, `recurring-task`. Companion note templates ship in the sample vault style under `_meta/templates/`. Command count: 36 → 42.
- **Task notes are now first-class, and a companion Task Graph plugin visualizes them.** The `type: task` frontmatter schema gains two fields in `references/ai-first-rules.md` and `references/vault-schema.md`: `area` (life area, free-form, suggested set god/self/family/service/professional/secular) and `depends-on` (a list of `[[Tasks/...]]` wikilinks this task is blocked by). `/obsidian-task` now ALWAYS creates a note in `Tasks/` with the full frontmatter (previously only "if substantial"), inferring `area` and `depends-on` so every task becomes a graph node. These fields feed a separate Obsidian plugin (repo `obsidian-task-graph`) that renders tasks as floating cards clustered by `area`, connected by shared project/person and by `depends-on` arrows. The plugin writes `status` back into the task note's frontmatter (mark a card done in Obsidian, future-Claude sees it next session) and stores card positions in its own data, keeping notes clean and AI-first. The vault markdown is the only contract between skill and plugin: no API, no server, fully portable.
- **Em-dash rule is now enforced.** Added `scripts/lint_emdashes.py` (check + `--fix`) and a versioned pre-commit hook at `.githooks/pre-commit` that blocks em-dashes in user-facing markdown prose, the rule CLAUDE.md always stated but nothing validated. The linter skips fenced code blocks and inline code spans, so documented filename conventions like `YYYY-MM-DD — slug.md` are preserved. Cleaned 571 pre-existing prose em-dashes across 54 files in one pass. Enable the hook per clone with `git config core.hooksPath .githooks`.
