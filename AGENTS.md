...
Operating instructions for Codex when working inside this repo.

This is the source repo for **obsidian-second-brain**, a Codex skill that turns any Obsidian vault into a living AI-first second brain. The skill ships 32 slash commands across 4 layers (vault management, thinking tools, research toolkit, scheduled agents).

If you are Codex operating on a user's vault, you want `_CLAUDE.md` inside their vault, not this file. This file is for working on the skill's source code.
...
Every command that writes to a user's vault MUST follow `references/ai-first-rules.md`. Vault notes are designed for **future-Codex retrieval**, not human reading. This means:

- A `## For future Codex` preamble at the top of every note
...
3. **Apply the AI-first rule to every vault write.** Reference `references/ai-first-rules.md` from the command body so future-Codex has the spec inline.
...
Symlink the local checkout into Codex so slash commands run from this repo:
