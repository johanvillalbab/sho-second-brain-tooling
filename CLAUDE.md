...
- **Markdown:** sentence-case headers (`## What it does`, not `## What It Does`). No emojis in command files. No em-dashes (`—`) in user-facing prose - use a regular dash or restructure the sentence. This is enforced by `scripts/lint_emdashes.py` and a pre-commit hook (see Testing locally). Em-dashes inside fenced code blocks and inline code spans are allowed (they protect documented filename conventions like `YYYY-MM-DD — slug.md`). Run `python3 scripts/lint_emdashes.py` to check, `--fix` to auto-correct prose.
...
Enable the repo's git hooks once per clone (blocks em-dashes in prose on commit):
